...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in pending_orders and pending_orders[user_id].get("step") == "waiting_receipt":
        ...
        await update.message.reply_text(
            ...
            f"قیمت: {item['price']}\n\n"
            f"زیر ۳۰ دقیقه تحویل داده میشه 🚀",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        if ADMIN_ID:
            try:
...
async def confirm_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=customer_id, text="✅ سفارش تأیید شد! به زودی تحویل داده میشه 🚀")
...
async def reject_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=customer_id, text="❌ سفارش تأیید نشد. با پشتیبانی تماس بگیر.")
